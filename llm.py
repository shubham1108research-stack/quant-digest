...
def _known_frameworks_block() -> str:
    """Prior art the novelty rubric measures a claimed contribution against.

    Prefers antecedents.json -- regenerated by tools/gen_antecedents.py from the
    curated classics now IN the archive, so it spans Method and Theory canon
    plus the most-cited established work, rather than canon.py's Method entries
    alone. Falls back to canon.py when the file is absent, so llm.py never
    depends on a generated artefact existing.

    Carries titles, authors, years and our own editorial notes only -- never the
    papers' text.
    """
    lines = []
    try:
        data = json.loads(pathlib.Path("antecedents.json").read_text(
            encoding="utf-8"))
        for a in data:
            who = " ".join(x for x in (a.get("authors"), a.get("year")) if x)
            tail = a.get("note") or (f"{a['cites']:,} citations"
                                     if a.get("cites") else "")
            lines.append(f"- {a['title']}"
                         + (f" ({who})" if who else "")
                         + (f": {tail}" if tail else ""))
    except Exception:                                  # noqa: BLE001
        lines = [f"- {title}: {why}" for papers in canon.CANON.values()
                 for (title, _a, _y, typ, why) in papers if typ == "Method"]
    return (
        "KNOWN, ALREADY-ESTABLISHED WORK (for calibration only -- this list is "
        "illustrative, not exhaustive; the same caution applies to any "
        "well-established framework or finding even if it isn't listed "
        "here):\n" + "\n".join(lines)
    )
