...
function renderWatched(){
  // everything from watched authors, newest first -- relevance is a shown
  // label, never a filter, so you judge each one yourself
  const q=$('q').value.toLowerCase().trim();
  const rows=DATA.filter(x=>x.watchlist)
    .filter(x=>!q||(x.title+' '+(x.authors||'')+' '+(x.watchlist_author||'')).toLowerCase().includes(q))
    .slice().sort(byDate);
  $('view').innerHTML=`<div class="dateline">Watched authors <span class="n">· ${rows.length} recent · relevance shown as a label, not a filter</span></div>`+
    (rows.length?rows.map(x=>entry(x)).join(''):'<div class="empty">Nothing from watched authors in the recent window yet.</div>');
}
...
function render(){VIEW==="monthly"?renderMonthly():VIEW==="foryou"?renderForYou():VIEW==="watched"?renderWatched():VIEW==="recent"?renderRecent():VIEW==="practitioners"?renderPractitioners():VIEW==="archive"?renderArchive():VIEW==="saved"?renderSaved():renderClassics();}
function setView(v){
  VIEW=v;['recent','foryou','watched','monthly','classics','practitioners','archive','saved'].forEach(k=>$('t-'+k).classList.toggle('on',k===v));
