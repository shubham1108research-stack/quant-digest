...
let EDGES=null, edgesLoading=false;
const EDGE_SIM=0, EDGE_CITE=1;
function loadEdges(){
  if(EDGES||edgesLoading)return Promise.resolve();
  edgesLoading=true;
  return fetch('edges.bin').then(r=>r.arrayBuffer()).then(buf=>{
    const dv=new DataView(buf), stride=13, n=Math.floor(buf.byteLength/stride);
    const adj=new Map();
    for(let i=0;i<n;i++){
      const o=i*stride;
      const a=dv.getUint32(o,true), b=dv.getUint32(o+4,true);
      const k=dv.getUint8(o+8), w=dv.getFloat32(o+9,true);
      if(!adj.has(a))adj.set(a,[]);
      if(!adj.has(b))adj.set(b,[]);
      adj.get(a).push([b,k,w]);
      adj.get(b).push([a,k,w]);          // both edge kinds are traversed either way
    }
    EDGES=adj;edgesLoading=false;
  }).catch(()=>{EDGES=new Map();edgesLoading=false;});
}
...
function expandGraph(seedRows, limit){
  ...
  seedRows.forEach((r,i)=>{
    ...
    for(const [nb,kind,w] of (EDGES.get(r)||[])){
      if(seed.has(nb))continue;
      const add=(kind===EDGE_CITE?1.0:Math.max(0,w))*decay;
      mass.set(nb,(mass.get(nb)||0)+add);
    }
  });
