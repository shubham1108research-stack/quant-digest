...
function nberEntry(x){
  ...
  return `<div class="entry"><div class="rail"><div class="yr">${esc((x.wp||'').replace(/^w/,'w '))}</div></div>
    <div class="body">
    <a class="title" href="${esc(x.url)}" target="_blank" rel="noopener">${esc(x.title)}</a>
...
function renderNBER(){
  ...
  $('view').innerHTML=`<div class="dateline">NBER · ${esc(label)} <span class="n">· ${rows.length} finance-program working papers · Asset Pricing · Corporate Finance · Monetary · Int'l Finance</span></div>`+
...
function render(){VIEW==="monthly"?renderMonthly():VIEW==="foryou"?renderForYou():VIEW==="nber"?renderNBER():VIEW==="recent"?renderRecent():VIEW==="practitioners"?renderPractitioners():VIEW==="archive"?renderArchive():VIEW==="saved"?renderSaved():renderClassics();}
function setView(v){
  VIEW=v;['recent','foryou','nber','monthly','classics','practitioners','archive','saved'].forEach(k=>$('t-'+k).classList.toggle('on',k===v));
