...
function entry(x,rank){
  ...
  const sc=(x.score!=null)?`<div class="rail">${rk}<div class="gauge" style="--pct:${x.score};--gc:${bandColor(x.score)}"><span>${x.score}</span></div><div class="cap">rating</div></div>`:`<div class="rail">${rk}</div>`;
