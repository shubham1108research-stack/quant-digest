...
async function doAsk(q,force){
  ...
  try{
    ...
    ASK_THREAD[0].scanned=cands.length;ASK_THREAD[0].state='reading';
    renderAsk();
    ...
    let extra=[],foundBy={};
    if(rest.length){
      ASK_THREAD[0].state='scanning';ASK_THREAD[0].scanning=rest.length;
      renderAsk();
      const batches=[];
    ...
    try{
      ...
      if(cand.length){
        ASK_THREAD[0].state='passages';ASK_THREAD[0].npsg=cand.length;
        renderAsk();
    ...
    ASK_THREAD[0].sources=psg.map(x=>Object.assign({},x.paper,
        {_sec:x.sec,_depth:'full'})).concat(picks);
    ASK_THREAD[0].extra=extra.length;
    ASK_THREAD[0].passages=psg;
    ...
    const ar=await fetch('/api/ask',{method:'POST',headers:{'content-type':'application/json'},
      body:JSON.stringify({mode:'answer',q:q,ctx:ctx})});
...
function renderAsk(notice){
  ...
  const chips=ASK_THREAD.length?'':'<div class="askex">'+ASK_EXAMPLES.map(e=>
  ...
  const thread=ASK_THREAD.map((t,ti)=>{
    ...
    const src=(t.sources&&t.state==='done')?'<div class="srch">Sources</div>'+t.sources.map((p,i)=>
      `<div class="src" id="src-${ti}-${i+1}"><span class="sn">${i+1}</span>
        ...
        <div class="meta">${p._depth==='full'?`<span class="ftag" title="full text — may support specification-level claims">full text${p._sec?' · '+esc(p._sec):''}</span>`:''}<span class="j">${esc(jlabel(p))}</span>${p.authors?' · '+esc(p.authors):''} · ${esc(p.date||p.seen||'')}${p.score!=null?' · rated '+p.score:''}</div></div></div>`
    ).join(''):'';
  ...
  $('view').innerHTML=`<div class="dateline">Ask the archive <span class="n">· ${VEC_UIDS?VEC_UIDS.length.toLocaleString():'—'} papers indexed · searched in your browser, ranked by similarity + your own paper scores + keyword fit; top ${ASK_DEEP} read in full and the next ${ASK_SCAN-ASK_DEEP} screened for relevant content</span></div>
