...
async function doAsk(q,force){
  ...
  try{
    ...
    const picks=deep.concat(extra);
    ASK_THREAD[0].sources=picks;ASK_THREAD[0].extra=extra.length;
    ...
    const ar=await fetch('/api/ask',{method:'POST',headers:{'content-type':'application/json'},
      body:JSON.stringify({mode:'answer',q:q,ctx:picks.map(p=>({title:p.title,authors:p.authors,
        ...
        depth:p._passages?'full':(p._full?'abstract':'summary_only')}))})});
...
function renderAsk(notice){
  ...
  const thread=ASK_THREAD.map((t,ti)=>{
    ...
    else if(t.state==='thinking')body='<div class="thinking">Synthesising from '+(t.sources||[]).length+' papers'+(t.extra?' ('+t.extra+' surfaced by the wider screen)':'')+(t.reused?' · '+t.reused+' from memory':'')+'…</div>';
